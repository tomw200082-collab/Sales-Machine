...
| **Architecture — DECIDED** | CTWA into the WhatsApp Business **app** on GT's existing work number (not the Cloud API, not Dualhook) | **CONFIRMED (Tom, 2026-08-30)** — `doctrine/decisions.md` **D-007**. Replaces the entry point only. `sales_core`, `lead_event`, the workspace, the conversion job and `/ingest` all carry over. Deletes the Cloud-API migration, the `referral` handler, the template stage and its budget, and **the Meta developer-access blocker of D-006** — an app-number CTWA ad needs Page admin + ad-account access, both of which GT has. Cost: no automatic capture from the app (Meta exposes no API to it; unofficial readers get the number banned), so the record layer is a phone shortcut + an optional link in the auto-greeting, both POSTing to `/ingest`. Runbook: 4 blocks, 19 steps. Assessment + amendment: `evidence/2026-08-30-ctwa-plan-impact.md` |
...
| U-014 | **MOSTLY CLOSED 2026-08-30 by D-007.** The Cloud-API access gap no longer gates anything: an app-number CTWA ad needs only Page admin + ad-account access. What remains, and cannot be checked from here: **is the WhatsApp number already linked to the GT Facebook Page, and do the Page and the number sit under the same Business Manager?** If yes the link is automatic with no code. The live test: if WhatsApp appears as a conversion destination in Ads Manager, this is closed. ~~Original: who administers the `Green Tea` Meta app and GT's ad account~~ D-006 proved GT holds none of it. Stages 1, 2 and 5 of Alexander's plan are all blocked on it, and unlike lead forms there is no Make-shaped way around it. This is a five-minute action by the right person | Alexander — proposed as task 0.0, ahead of his own stage 0 |
| U-015 | **CLOSED 2026-08-30 by D-007.** GT's work number, already a WhatsApp Business account with a catalog, and already separate from the Dualhook order-intake number. No conversion, no second SIM, no risk to the order channel's quality rating. Residual watch item, not a blocker: if existing customers also message this number, ad traffic shares the inbox — labels handle the ordering; accumulating blocks would be the signal to split. ~~Original: one number or two~~ GT already has a number in the API carrying B2B order intake. Sharing it means the CTWA auto-reply and the order bot receive one stream and must not answer over each other; separating it means two numbers, two webhooks, two quality ratings. The plan's own rule ("never put a customer-facing number in the API") was written without knowing one already is | Tom + Alexander — business decision; assessment §4.1 |
...
| U-019 | **MOOT 2026-08-30 by D-007.** Replies are manual from the app, so there are no templates, no per-message tariff and no 72-hour window to model — the operational budget is zero. Re-opens only if the number ever moves to the Cloud API. ~~Original: unverified in the plan's cost model:~~ Meta's current 72-hour free-entry-point behaviour, and the Israel marketing-template tariff. The whole operational budget (task 5.3) rests on both. Reproduced from the plan, not confirmed against Meta's live pricing docs — rule 4 says check before writing | Technical — read Meta's pricing documentation directly, before any budget is approved |
| U-020 | **CLOSED 2026-08-30.** `LEAD_INGEST_TOKEN` is set and correct — proven by live `source='facebook'` rows landing daily; an unauthenticated POST returns 401 and writes nothing. ~~Is it set, and what is the function's full URL?~~ Both the phone shortcut and the greeting-link form POST to `/ingest` and need them. Cheap to check; nothing is built until it is | Technical |
| U-021 | **What stops the app path becoming the permanent architecture by default?** D-007 is right for a pilot. Set a tripwire now — a weekly lead volume, or the point a second person must answer routinely — at which the Cloud-API question is re-opened deliberately rather than by drift | Tom |
| U-022 | The auto-greeting wording, the quick-reply answers, and whether a short enrichment link belongs in the greeting. All three depend on the workbook (U-018), which this repo has still never seen | Alexander, via Tom |
| U-023 | **Recover the 2026-08-10 → 2026-08-23 hole.** Export that range from Meta Lead Center and POST to `/ingest` (idempotent). Expires from Meta in early November | Tom exports, Claude ingests |
| U-024 | Inspect the 2 dead-letter items on Make scenario 7075235 — each is a lead that entered and did not complete | Technical |
| U-025 | **Freshness failure, not a one-off.** A `CURRENT_STATE.md` line was true when written and false hours later, and nothing swept it; six days on it produced a confidently wrong answer. Rule 6 requires staleness sweeps — what actually triggers one? | Tom |
