...
| U-038 | **ימי המשלוח סותרים את עצמם.** טום ענה `2026-09-02`: `פשוט נכתוב עד 3 ימי עסקים- זה מכסה הכל נכון?` — אבל D-010(ו) מ-`2026-08-31` כבר קבע את **הימים** כהבטחה (`מרכז א׳/ב׳/ה׳ · צפון ג׳ · דרום ד׳`), ובמדידה `עד 3 ימי עסקים` מכסה **רק את המרכז**: צפון ודרום מקבלים יום מסלול אחד בשבוע, ולקוח שמפספס אותו ממתין עד 7 ימים. המחסן ⊥ הצוואר — 50 ההזמנות האחרונות: חציון `21.6` שעות מהזמנה ליציאה, `82%` תוך יום עסקים אחד, `100%` תוך שניים. הבטחה ארצית של 3 ימים תישבר אצל כל לקוח בצפון ובדרום שמפספס את יומו. **⊥ נרשם כדוקטרינה עד שטום מכריע בין השניים.** | טום — הכרעה בין D-010(ו) לבין נוסח ארצי |
