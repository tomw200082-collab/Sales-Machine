...
| U-039 | **`ספר העבודה §07` מצוטט 15 פעמים ואינו קיים.** `answer-bank` · `sales-motion` · `buyers` · `refusals` · `public-claims` · `chapters` · `questions` מצטטים אותו כמקור, בדרגה `doc_confirmed`. `ספר העבודה` = `/Business/Gt Knowledge Base` בדרופבוקס (טום 2026-09-02) — ממוספר `00`–`06`, אין בו `§07`, והוא **הפלט של הריפו** ולא מקור עצמאי (`נוצר אוטומטית · מקור: Sales-Machine/knowledge/`). `GT_Playbook_HE_.xlsx` הוא המדריך לעובד של מערכת הייצור, בלי תשובות לקוח. ∴ 15 ציטוטים מדורגים מול מסמך שאי אפשר לפתוח. גם: העותק בדרופבוקס מ-`2026-08-31T16:23Z` מפגר יומיים אחרי הריפו (`0ea90d3`) | טום — לאתר את המסמך, או להוריד את 15 הציטוטים ל-`user_confirmed` |
