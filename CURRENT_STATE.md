...
| U-020 | Is `LEAD_INGEST_TOKEN` set in the `sales-leads-poll` environment, and what is the function's full URL? Both the phone shortcut and the greeting-link form POST to `/ingest` and need them. Cheap to check; nothing is built until it is | Technical |
