...
| U-034 | **CLOSED 2026-09-02 — Tom confirmed the empty channel is his.** `UC8Wlby9ihV04Ky5CsZZoWWg` (`GT everyday`, no avatar, no videos) appears under his own Switch account, so `youtube.com/@gteveryday` is GT's to move. **The TikTok pattern a third time: a property created, never used, and forgotten.** GT's real channel stays `UC7U7qL5vs9xt6Rzxj2GiSOg` (7 videos); the empty one is renamed to release the handle and **kept, never deleted** | closed — the handle move is a task, not an unknown |
