...
| U-033-a | **CLOSED as diagnosed, 2026-09-02 — verifying the domain was never going to fix it, and the code says so in a comment.** The 03:00Z run failed with the byte-identical `resend 403`: due 12, sent 1, **11 undelivered to Avi** (9 the day before — 20 across two days, all recorded as sent). Root cause is in `supabase/functions/sales-leads-poll/index.ts:60`: `const DEFAULT_FROM = 'GT Leads <onboarding@resend.dev>'`, used at line 277 unless the `RESEND_FROM` secret is set. `onboarding@resend.dev` is **Resend's shared test sender, which Resend restricts to the account owner's own address** — that is the exact 403. A verified domain changes nothing while the `from` still points at the shared sender. The comment above that line states the assumption that expired: *"Resend's shared sender works before any DNS work … because the only recipient is the account owner."* True when Tom was the only recipient; false the moment Avi became an assignee, and nobody revisited it. **This is wider than the digest** — `poll_run` shows the same `resend 403` on the **`ingest`** route on 2026-08-31 and 2026-09-01, so **new-lead alerts to anyone but Tom have been failing too** | **CORRECTED 2026-09-03 — "the now-verified domain" was wrong; no domain is verified in Resend.** `resend._domainkey.gteveryday.com` does not resolve (DoH lookup, 2026-09-03), so the secret alone would still 403. It is **two steps, in order**: (1) verify a domain in Resend and paste its records at the DNS host — nameservers are `ns27/ns28.domaincontrol.com`, i.e. **GoDaddy**; use the subdomain `send.gteveryday.com`, because the root SPF already carries two `include:` (a GoDaddy SPF-merge and `spf02.hubspotemail.net`) and a third moves it toward SPF's 10-lookup ceiling, which would break Google Workspace mail for the whole domain. (2) set `RESEND_FROM` to an address on it, e.g. `GT Leads <leads@send.gteveryday.com>`. **Now readable without waiting for 03:00Z:** `POST {"route":"health"}` returns `RESEND_FROM` and `sends_from_shared_sender` as of `sales-leads-poll` v20, deployed 2026-09-03 — reading `false` / `true` at that moment. The 06:00 IL run of 2026-09-03 was the third: due 14, sent 1, **13 more claimed and undelivered — 33 across three days** | Tom — the two steps above. ⊥ code, ⊥ deploy |
